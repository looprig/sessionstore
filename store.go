...
func Open(ctx context.Context, backend *storage.Composite, opts ...Option) (*Store, error) {
	...
	return &Store{
		...
		ctx:            ownedCtx,
		cancel:         cancel,
		providerCloser: cfg.providerCloser,
		closeDone:      make(chan struct{}),
	}, nil
}

// Close initiates shutdown exactly once. It cancels Store-owned work, waits for
// it, then closes an explicitly owned provider at most once. The initiating
// caller's ctx is passed to that provider close; every caller's ctx bounds its
// own wait. If a caller stops waiting, shutdown continues and a later call can
// observe the one stable final result.
	...
	s.closeOnce.Do(func() {
		...
		go func(closeCtx context.Context) {
			...
			if s.providerCloser != nil {
				s.closeErr = s.providerCloser.Close(closeCtx)
			}
			close(s.closeDone)
		}(ctx)
	})
